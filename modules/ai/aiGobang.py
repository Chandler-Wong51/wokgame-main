...
class aiGobang():
    def __init__(self, ai_color, player_color, search_depth=1, **kwargs):    #初始化
        ...
        self.alpha = -99999999
        self.beta = 99999999
        self.all_list = [(i, j) for i, j in product(range(19), range(19))]    #生成全部列表
    '''外部调用'''
    def act(self, history_record):
        self.ai_list = []
        self.player_list = []
        self.aiplayer_list = []
        for item in history_record:     #把历史记录加载进来
        ...
        while True:
            self.next_point = random.choice(range(19)), random.choice(range(19))    #随即找到一个点
            if self.next_point not in self.aiplayer_list:    #下一个点必须是未被下过的点
                break
    ...
    def __doSearch(self, is_ai_round, depth, alpha, beta):
        ...
        blank_list = list(set(self.all_list).difference(set(self.aiplayer_list))) #未下列表
        blank_list = self.__rearrange(blank_list)    #重新排列
